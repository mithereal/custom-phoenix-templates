defmodule Mix.Tasks.Default.Gen.Html do
  @shortdoc "Generates controller, views, and context for an HTML resource"

  @moduledoc """
  Generates controller, views, and context for an HTML resource.

      mix default.gen.html Accounts User users name:string age:integer
  ...
  Overall, this generator will add the following files to `lib/`:
    ...
    * a view in lib/app_web/views/user_partials_view.ex
    * a controller in lib/app_web/controllers/user_controller.ex
    * default layout templates in lib/app_web/templates/user
    * default CRUD templates in lib/app_web/templates/user/partials/
  ...
  Alternatively, the `--context-app` option may be supplied to the generator:
      ...
      mix default.gen.html Sales User users --context-app warehouse
  ...
  module name, for example:
      ...
      mix default.gen.html Sales User users --web Sales
  ...
  a `--table` option may be provided. For example:
      ...
      mix default.gen.html Accounts User users --table cms_users
  ...
  Read the documentation for `default.gen.schema` for more information on
  attributes.
  """
  use Mix.Task

  alias Mix.Phoenix.{Context, Schema}
  alias Mix.Tasks.Default.Gen

  @doc false
  def run(args) do
    if Mix.Project.umbrella? do
      Mix.raise "mix default.gen.html can only be run inside an application directory"
    end
  ...
  def files_to_be_generated(%Context{schema: schema, context_app: context_app}) do
    ...
    [
      ...
      {:eex, "edit.html.eex",       Path.join([web_prefix, "templates", web_path, schema.singular, "partials/edit.html.eex"])},
      {:eex, "form.html.eex",       Path.join([web_prefix, "templates", web_path, schema.singular, "partials/form.html.eex"])},
      {:eex, "index.html.eex",      Path.join([web_prefix, "templates", web_path, schema.singular, "partials/index.html.eex"])},
      {:eex, "new.html.eex",        Path.join([web_prefix, "templates", web_path, schema.singular, "partials/new.html.eex"])},
      {:eex, "show.html.eex",       Path.join([web_prefix, "templates", web_path, schema.singular, "partials/show.html.eex"])},
      {:eex, "view.ex",             Path.join([web_prefix, "views", web_path, "#{schema.singular}_view.ex"])},
      {:eex, "partials_view.ex",    Path.join([web_prefix, "views", web_path, "#{schema.singular}_partials_view.ex"])},
  ...
  def copy_new_files(%Context{} = context, paths, binding) do
    ...
    Mix.Phoenix.copy_from(paths, "priv/templates/default.gen.html", binding, files)
